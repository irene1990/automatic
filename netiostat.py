...
while (count > 0):
    ...
    line = os.popen(netcmd).readline().strip()
    netio = getnetio(line)
    info.append(u'网络入总量:%.4fm,网络出总量:%.4fm'%(netio[0]/1024/1024,netio[1]/1024/1024))
    time_curr = time.time()
    ...
    info.append(u'当前网络流入速度:%.4fk/s'%(neti_total/sec_total/1024))
    info.append(u'当前网络流出速度:%.4fk/s'%(neto_total/sec_total/1024))
    show = ",".join(info)
